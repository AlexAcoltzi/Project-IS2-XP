...
void MainWindow::on_cambiarC_clicked(){
    ...
    if (query.exec("update Usuario set contrasena = '"+password2+"' where usuario = '"+username+"' and contrasena= '"+password+"';")){
...
void MainWindow::TablaReservacion(){
    ...
    QString reserv = "SELECT inicio, fin, total_a_pagar, liquidado  FROM Reservacion "
    ...
    while(qry.next()){
        ...
        QString prec = "$" + qry.value("total_a_pagar").toString();
...
void MainWindow::on_reservarcajon_3_clicked(){
    ...
    if(llenocaj111 == 1){
        ...
        qDebug() << tiempoInicio << " " << tiempoFinal;
...
void MainWindow::on_aceptar_clicked(){
    ...
    if(calle.isEmpty()||colonia.isEmpty()||cp.isEmpty()||no_interior.isEmpty()||no_exterior.isEmpty()){
        QMessageBox::about(this, "Error", "Campos Vacios");
        return;
    }
...
void MainWindow::Piso(){
    QString dato,  num, tipo, nombre, agno, mes, cvv, id, id2, id3, tipo2, numPiso,pago;
     ...
     dato="select idCajon, pagoxhora, inicio, fin from Reservacion where idCliente='"+id2+"' and estado='0'";
     if (query.exec(dato)){
         while (query.next()){
             num = query.value(0).toString();
    ...
    if (ok){
        ...
        if((QString::compare(cvv,Dialogo))!=0){
            ...
            qDebug()<<s;
             ...
             if(!cajonE.exec("update cajon set lleno='0' where idCajon='"+id3+"';")){
             ...
             QSqlQuery sql;
             QString estado;
             estado="update Reservacion set estado = '1' where idReservacion='"+idReserva+"';";
             qDebug()<< estado;
             if(!sql.exec(estado)){
...
void MainWindow::on_crearOferta_clicked(){
    ...
    if(ui->aniversario->isChecked()){
        if((tiempo>= aniversario) && (tiempo<=aniversario2)){
            ui->img_oferta->setStyleSheet("border-image: url(:/Imagenes/camara.png);");
...
void MainWindow::on_Pagar_3_clicked()
{
    ...
    if (ok){
        ...
        if((QString::compare(cvv,Dialogo))!=0){
            ...
            QString rPagoPen="UPDATE transaccion SET liquidado='1' WHERE idPension='"+idPen+"'";
